...
let type_declarations ?(equality = false) env name decl1 id decl2 =
  ...
  || decl2.type_kind = Type_abstract && decl2.type_manifest = None then begin
    if not (List.for_all2
              (fun (co1,cn1,ct1,i1) (co2,cn2,ct2,i2) -> (not i2 || i1))
              decl1.type_variance decl2.type_variance)
    then Location.prerr_warning decl1.type_loc
           (Warnings.Injectivity 
              (name,
               (Location.to_string decl1.type_loc),
               (Location.to_string decl2.type_loc)));
    if List.for_all2
        (fun (co1,cn1,ct1,i1) (co2,cn2,ct2,i2) ->
          (not co1 || co2) && (not cn1 || cn2))
        decl1.type_variance decl2.type_variance
    then [] else [Variance]
    end
  else []
